...
<script type="text/javascript">
	...
	function getShopsList(signed_text){
		...
		var path = "";
		if(signed_text == ""){
			path = "<%= path%>/api/v1/communities/${communityId}/shops?pageNum="+signedThisPageNum+"&pageSize="+signedThisPageSize;
		}else{
	...
	function getServiceCategoryList(){
		var path = "<%= path%>/api/v1/communities/shoptype";
				$.ajax({
					...
					success:function(data){
						console.info(data.info[0].categoryId+ "----------------------");
						for(var i = 0 ; i < data.info.length ; i++){
							console.info(data.info[i].shopTypeId+"-------"+data.info[i].shopTypeName);	
							serviceMap[parseInt(data.info[i].shopTypeId)] = data.info[i].shopTypeName+"";
						}
						console.info(serviceMap);
					},
...
<body>
	...
	<div class="signed-search">
		...
		<a href="javascript:creatShop();" class="blue-button">创建一个新的店家    +</a>
		<a href="javascript:;" class="blue-button">查看所有未开通的验证码</a>
	</div>
	<div class="signed-list">
		<ul id="singed-list-ul">
	...
	<div style="float: left; width:  100% ; height: 50px; clear: both; text-align: center;">
		...
		当前第 <font id="signedPageNum"></font> 页
		共 <font id="signedPageSize"></font> 页
		共 <font id="signedSum"></font> 条
	...
	<div class="signed-creat-box" style="display:none;">
