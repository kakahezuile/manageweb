...
function countFacilitiesTop(){
		...
		var path = "<%= path%>/api/v1/communities/"+communityId+"/facilities/countFacilitesTop";
		...
		$.ajax({
			...
			success : function(data) {
              ...
              //document.getElementById("nearby_take_out_num").innerHTML=data["KTV"];  
	...
	function selectFacilitiesName(pageNum) {
			...
			var path = "<%= path%>/api/v1/communities/"+communityId+"/facilities/selectFacilits?pageNum="
				+ pageNum + "&pageSize=5&facilitiesName=" + facilitiesName;
		...
		$.ajax({
					...
					success : function(data) {
						...
						for ( var i = 0; i < data.length; i++) {
							...
							liList += "<span>" + data[i].facilityName
									+ "</span>（" + data[i].phone + "）</div>";
							liList += "<div class=\"shop-position\"><span>位置："
									+ data[i].address + "</span>坐标:"
									+ data[i].longitude + ":"
									+ data[i].latitude;
							liList += "</div></div><div class=\"shop-type\"><span>类型：</span>"
									+ data[i].typeName + "</div>";
							...
							if(data[i].status=="normal"){
							  liList += "<a href=\"javascript:;\" class=\"edit\">编辑</a><a href=\"javascript:;\" class=\"red-button\" id=\"facilities_status_"+data[i].facilityId+"\" onclick=\"upFacilities('"+data[i].facilityId+"','"+data[i].status+"');\">屏蔽</a>	</li>";
							  
							}else{
							
							liList += "<a href=\"javascript:;\" class=\"edit\">编辑</a><a href=\"javascript:;\" class=\"green-button\" id=\"facilities_status_"+data[i].facilityId+"\" onclick=\"upFacilities('"+data[i].facilityId+"','"+data[i].status+"');\">开启</a>	</li>";
	...
	if(fac_status=="red-button"){
	 ...
	 status="normal";
			...
			var path = "<%= path%>/api/v1/communities/${communityId}/facilities/"
				+ facilityId;
		...
		$
				.ajax({
					...
					success : function(data) {
						if (data.status == "yes") {
							document.getElementById("facilities_status_"
									+ facilityId).className = but;
							document.getElementById("facilities_status_"
									+ facilityId).innerHTML = but_ti;
						}
					},
					error : function(er) {
					}
	...
	function getNearbyList(num) {
		...
		var repairRecordPageNum = document
				.getElementById("nearby_list_number_get").value;
		var repairRecordPageSize = document
				.getElementById("nearby_list_sizepage_get").value;
		if (num == -1) { // 上一页
				...
				<div class="nearby_right-body">
					<div class="main clearfix">
						<div class="nearby-total clearfix">
							<ul id="top_nearby_id">
								<li>
									<p class="number" id="nearby_cvs_num">
										0
									</p>
									<p class="name">
										便利店
								...
								<li>
									<p class="number" id="nearby_drugstore_num">
										0
									</p>
									<p class="name">
										药店
								...
								<li>
									<p class="number" id="nearby_take_out_num">
										0
									</p>
									<p class="name">
										外卖
						...
						<div class="nearby-search">
							<input type="text" placeholder="搜索店家  "
							...
							<ul id="nearby_list">
								<li>
									<div class="shop-image">
										<img
											src="${pageContext.request.contextPath }/images/nearby/shopimage1.png" />
									</div>
								...
								<li>
									<div class="shop-image">
										<img
											src="${pageContext.request.contextPath }/images/nearby/shopimage2.png" />
									</div>
									<div class="shop-detail">
										<div class="shop-name">
