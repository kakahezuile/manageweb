...
<script type="text/javascript">
		...
		activity_Title.append("<input id=\"activity_Title"+activityId+"\" value=\""+activityTitle+"\" type=\"hidden\"/>");
		 if(status == "ongoing"){
			if(activities_sum % 2 == 0){
				...
				activitesUl.append("<tr  class=\"odd\"><td><a  href=\"<%=path %>/jsp/estate/lifecircle/lifecircle-detail.jsp?activitiesId="+activityId+"\">"+activityTitleTemp+"</a></div></td><td><div title=\""+activityDetailTemp+"\" class=\"lifecircle-user\"><p>刘文静</p><p>3号楼2单元102</p></div></td><td class=\"pic\">"
				
				+array_element_ar+
				
				"</td><td class=\"activity-opeation\"><a id=\"activities_aid_"+activityId+"\" href=\"javascript:;\" onclick=\"selectActivityMessage('"+emobGroupId+"');\"  class=\"refuse\">查看内容</a></td></tr>");
				...
				activitesUl.append("<tr class=\"even\"><td><a  href=\"<%=path %>/jsp/estate/lifecircle/lifecircle-detail.jsp?activitiesId="+activityId+"\">"+activityTitleTemp+"</a></div></td><td><div title=\""+activityDetailTemp+"\" class=\"lifecircle-user\"><p>刘文静</p><p>3号楼2单元102</p></div></td><td class=\"pic\">"
	...
	function selectActivityMessage(emobGroupId){
		$("#monitorActivityMessage").css("display","block");
		$("#monitorActivityDiv").css("display","none");
		...
		$.ajax({
			...
			success:function(data){
				if(data.status == "yes"){
					data = data.info;
					var len = data.length;
					var monitorActivityTable = $("#monitorActivityTable");
					monitorActivityTable.html("");
					for(var i = 0 ; i < len ; i++){
						monitorActivityTable.append("<tr><td>"+data[i].nickname+": "+data[i].msg+"</td></tr>");
					}
				}
			}
		});
	...
	function activities_search(){
		...
		var activities_search_text = $("#activities_search_text_detil").val();
			getActivitiesList(1,activities_search_text,"ongoing");
	}
	...
	function getMyActivityList(num){
		...
		getActivitiesList(activityThisPageNum,activities_search_text,"ongoing");
	}
...
<div class="content clearfix" >
	...
	<div class="activity-right-body">
		...
		<div id="monitorActivityMessage" style="height: 660px;overflow:scroll;">
			<table id="monitorActivityTable" >
			
			</table>
		</div>
		
		<div id="monitorActivityDiv">
...
<script type="text/javascript">
	$("#monitorActivityDiv").css("display","block");
	$("#monitorActivityMessage").css("display","none");
	montiorin(5);
	getActivitiesList(1, "", "ongoing");
</script>
</html>