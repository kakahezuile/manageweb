...
<html class="">
	...
	<head>
		...
		<script src="${pageContext.request.contextPath }/js/estate.js"></script>
				...
				success:function(data){
					...
					if(data.status == "yes"){
						...
						$("#express_table_fei").empty();
							$("#express_table_fei").append("<tr><th>电话</th><th>收件人</th><th>物品</th><th>通知结果</th><th>用户类型</th><th>收件时间</th><th>取件</th></tr>");
						...
						for(var i = 0 ; i < len ; i++){
							...
							if(pageData[i].expressStatus == 1){
								...
								$("#express_table_fei").append("<tr class='odd'><td>"+pageData[i].expressPhone+"</td><td>"+pageData[i].expressName+"</td><td>"+pageData[i].expressGoods+"</td><td>"+expressResult+"</td><td>"+expressUserType+"</td><td>"+getSmpFormatDateByLong(expressCollectionTime,false)+"</td><td>"+express_status_html+"</td></tr>");
					
							}else{
								express_status_html = "	<a href=\"javascript:;\"  onclick=\"updateExpress("+pageData[i].expressId+");\">确认收件</a>";
								$("#express_table_fei").append("<tr class='even'><td>"+pageData[i].expressPhone+"</td><td>"+pageData[i].expressName+"</td><td>"+pageData[i].expressGoods+"</td><td>"+expressResult+"</td><td>"+expressUserType+"</td><td>"+getSmpFormatDateByLong(expressCollectionTime,false)+"</td><td>"+express_status_html+"</td></tr>");
	...
	document.getElementById("express_message_inform_fei").value="尊敬的"+username+"您好，帮帮为您代收了一件快递，请您到物业处领取。注册成为帮帮会员，免费享受更多服务（http://www.）";
	
	}
	
	
	
function getExpressListByText2(){
			...
			<div class="content clearfix">
				...
				<div class="express-right-body">
					...
					-->
						...
						<a class="del-master"
							href="<%=path %>/jsp/estate/express/express_place.jsp" >收件地址设置</a>
					</div>
					<div class="express-site-message">
						...
						<input id="express_station_inform_fei"
							value="报告XXX帮主，帮帮为您代收了一份快递，现存放于小区东门保安亭。领取时间为（7:30-22:30）请记得取回" type="text" />
					</div>
					<div class="express-phone-message">
						<label>
							短信通知
						</label>
						<input value="尊敬的XXX：帮帮 为您代收了一件快递，现存放于小区东门保安亭。注册成为帮帮会员，免费享受更多服务（http://114.215.114.56:8080/jsp/app/download.jsp）"
					...
					<div class="express-user-info">
						...
						<a class="express-recive-button" href="javascript:;" id="express_submit_fei" onclick="expressSubmit();">收件并通知</a>
					</div>
					<div class="express-list">
						<div class="express-list-head">
							...
							<div class="express-search">
								<input type="text" id="express_search_fei_id" onkeydown="if(event.keyCode==13){getExpressListByText2();}" placeholder="搜索  电话/收件人"/>
								<a href="javascript:void(0);" onclick="getExpressListByText2();">查找</a> 
							</div>
