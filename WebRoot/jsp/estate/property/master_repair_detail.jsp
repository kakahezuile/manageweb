				...
				$.ajax({
		                ...
		                if(sort=='5'){
		                       ...
		                       if(isComplaints=="no"){
		                           ...
		                           liList+=data.shopName+"维修列表</p>";
		                       }
                               liList+="<p>维修次数<span>"+(data.repairCountHistory+data.repairCountHistoryPart)+"</span></p>";
                               liList+="<p>分包维修次数<span>"+data.repairCountHistoryPart+"</span></p>";
                                liList+="<p> 拒绝次数<span>" +data.unComplaintsCount+"</span></p>";
                                liList+="<p> 放弃次数<span>" +data.abandonCount+"</span></p>";
...
function masterRepairDatailList(startTime,endTime,pageNum,shopId,sort,isComplaints){
		...
		$.ajax({
					...
					success : function(data) {
						...
						if(sort=='5'){
							repair_overman
								.append("<tr><th class='repair-record-user-face'>客户头像</th><th>客户昵称</th>"
										+ "<th>客户电话</th><th>维修类型</th><th>维修金额</th><th>接单时间</th><th>服务时长"
										+ "</th><th>订单状态</th></tr>");
						...
						for ( var i = 0; i < data.length; i++) {
							...
							liList += "	<td class='repair-record-user-face'><img src='"+data[i].avatar+"'/></td>";
							liList += "<td class='repair-record-user-name'><div>" + data[i].nickname + "</div><div class=\"user-room\">" + data[i].userFloor+data[i].userUnit+data[i].room + "</div></td>";

							liList += "<td class='repair-record-user-name'>" + data[i].username + "</td>";
							
							if(sort=='5'){
							      if(data[i].orderType=="bangbang"){
							      
							         liList+= "</td><td>物业维修</td>";
							      }else{
							         liList+= "</td><td>分包维修</td>";
							      }
							...
							if(data[i].startTime<data[i].endTime){
						    	...
						    	var date3=(data[i].endTime*1000)-(data[i].startTime*1000);
						    	var leave1=date3%(24*3600*1000) ; 
						    	
						    	var leave2=leave1%(3600*1000) ;       //计算小时数后剩余的毫秒数
                                   // hours=Math.floor(leave2/(60*1000)); 
                                hours=Math.floor(leave1/(60*1000));
                                 
                                 hours=((date3/1000)/60);
                                 if(hours<60){
                                    hours=hours.toFixed(2)+"分钟";
                                 }else if(hours>(60*24)){
                                    hours=hours/(60*24);
                                    hours=hours.toFixed(2)+"天";
                                 }else{
                                    hours=hours/60;
                                    hours=hours.toFixed(2)+"小时";
                                 }
							   	...
							   	var date3=((new Date()).getTime())-(data[i].startTime*1000);
						    	var leave1=date3%(24*3600*1000) ; 
							...
							var myDate = new Date();
							liList += "<td>" + "维修金额" + "</td>";
							var s=myDate.getFullYear()+"年"+(myDate.getMonth()+1)+"月"+myDate.getDate()+"日";
							
							liList += "<td>" + s + "</td><td>"+hours+"</td>";
							if(data[i].status=="new"){
									liList += "<td class='user-complain'>未接订单</td></tr>";
							}else if(data[i].status=="ended"){
							  liList += "<td class='user-complain'>已结单</td></tr>";
							}else if(data[i].status=="refuse"){
							 liList += "<td class='user-complain'>已拒绝</td></tr>";
							}else if(data[i].status=="ongoing"){
							 liList += "<td class='user-complain'>进行中</td></tr>";
							}else if(data[i].status=="renounce"){
							 liList += "<td class='user-complain'>已放弃</td></tr>";
  ...
  //日历控件
				...
				<div class="service-right-body">
					...
					<div id="day_top_type"></div>
					  ...
					  <div class="time-chose" id="top_section_time" style="display: none;">   
						...
						<div class="repair-title" style="display: none;" id="day_top">
	...
	function isDay(type){
	  ...
	  if(type=="month"){
	    document.getElementById("day_top_type").innerHTML="本月";
        ...
        document.getElementById("top_day_type").innerHTML=sta+"至"+end;
	    document.getElementById("day_top_type").style.display="本日";
	    masterRepairDatailId3("<%=startTime%>", "<%=endTime%>","<%= shopId%>", "<%=sort%>","<%=isComplaints%>");
	  }else if(type=="day"){
	   document.getElementById("day_top_type").style.display="本日";
	    masterRepairDatailId3("<%=startTime%>", "<%=endTime%>","<%= shopId%>", "<%=sort%>","<%=isComplaints%>");
	  }else{
	     document.getElementById("day_top").style.display="";
	     document.getElementById("day_top_type").style.display="none";
