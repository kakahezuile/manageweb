...
<meta name="Description" content="小间物业,小间物业管理系统" />
	...
	<script type="text/javascript">
		...
		function getSensitiveWords(pageNum){
			...
			var path = "<%= path%>/api/v1/communities/"+communityId+"/lifeCircle/getSensitive?pageNum="+pageNum+"&pageSize="+sensitiveWordsPageSize;
				...
				success:function(data){
				  var data2=data.info;
			        ...
			        $("#sensitiveMain").attr("style","display:none");
						...
						var len = data.length;
						for(var i = 0 ; i < len ; i++){
							outTime = getSmpFormatDateByLong(data[i].createTime * 1000);
							var status = "";
							var className ="even";
							if(data[i].status != "dispose"){
								status = "未处理";
							}else{
							  status = "已处理";
							}
							...
							$("#sensitiveWordsTable").append("<tr class=\""+className+"\"><td>"+data[i].detailContent+"</td><td>"+outTime+"</td><td>"+status+
							"</td><td class=\"sensitive-detail\"><a class=\"red-button\" href=\"javascript:void(0);\" onclick=\"getActivitiesDetail('"+data[i].lifeCircleId+"','"+data[i].detailContent+"','"+data[i].id+"');sensitiveWordsSelect("+i+");examine('"+data[i].id+"','"+data[i].lifeCircleId+"','"+data[i].detailContent+"');\" >查看</a></td></tr>");
						}
		...
		function getSensitiveWordsByStatus(status){
			...
			if(status == "next"){
				...
				//sensitiveTableFirst.before("<li><td>22222222</td></li>");
						...
						for(var i = 0 ; i < len ; i++){
							...
							if(status == "next"){
								...
								if(data[i].messageFrom == sensitiveEmobId){  // 左边
									...
									sensitiveTableNext.after("<li><span>"+nickName+"</span>: "+msg+"</td></li>");
								}
								//sensitiveTableNext.after("<li><td>11111111</td></li>");
							}else if(status == "last"){
								if(i == len-1){
								...
								if(data[i].messageFrom == sensitiveEmobId){  // 左边
									sensitiveTableFirst.before("<li><span>"+nickName+"</span>: "+msg+"</td></li>");
								}else{ // 右边
									sensitiveTableFirst.before("<li><span>"+nickName+"</span>: "+msg+"</td></li>");
								}
								//sensitiveTableFirst.before("<li><td>22222222</td></li>");
		...
		function sensitiveWordsSelect(index){
			...
			$("#sensitiveTop").empty();
			$("#sensitiveTop").append("<table align=\"center\" width=\"65%\" height=\"100%\" ><tr><td>敏感词<span>"+data.sensitiveWords+"</span></td></td><td>出现时间<span>"+outTime+"</span></td><td class=\"close-box\"><a class=\"close-activity-group\" href=\"javascript:;\" onclick=\"deleteActivities('"+sensitiveGroupId+"','"+sensitiveEmobId+"')\" >无效敏感词</a><a class=\"close-activity-group\" href=\"javascript:;\" onclick=\"deleteActivities('"+sensitiveGroupId+"','"+sensitiveEmobId+"')\" >隐藏</a></td></tr></table>");
		...
		function examine(id,lifeCircleId,detailContent){
			...
			$.ajax({
				...
				success:function(data){
		              //	window.location="/jsp/estate/lifecircle/lifecircle-detail-word.jsp?activitiesId="+lifeCircleId+"&id="+id+"&detailContent="+detailContent;	
				},
	...
	function deleteActivities(id,emobGroupId){
		      ...
		      function getActivitiesDetail(activitiesId,sensitiveWords,id){
				...
				$.ajax({
					...
					document.getElementById("sensitiveMain").style.display="";
					document.getElementById("sensitiveWordsTable").style.display="none";
					...
					document.getElementById("user_name_id").innerHTML=data.nickname;
					document.getElementById("neirong").innerHTML=data.lifeContent;
						var myDate=new Date(data.createTime*1000);
					
					
					$("#sensitiveTop").empty();
			      $("#sensitiveTop").append("<table align=\"center\" width=\"65%\" height=\"100%\" ><tr><td>敏感词<span>"+sensitiveWords+"</span></td></td><td>出现时间<span>"+myDate.getFullYear()+"-"+(myDate.getMonth()+1)+"-"+myDate.getDate()+"</span></td><td class=\"close-box\">"+
			      "<a class=\"close-activity-group\" href=\"javascript:;\" onclick=\"wuxiao('"+id+"')\" >无效敏感词</a><a class=\"close-activity-group\" href=\"javascript:;\" onclick=\"examine2('"+id+"','"+activitiesId+"')\" >隐藏</a></td></tr></table>");
					
					
					 
					lifecirel(activitiesId,data.nickname,sensitiveWords);
	         ...
	         function lifecirel(emob_id_to,thisName,sensitiveWords){
	        var path = "<%= path%>/api/v1/communities/1/lifeCircle/getLifeCircleDetail?emob_id_to="+emob_id_to;
				$.ajax({
					...
					success:function(data){
					data=data.info;
					var activitesDate=$("#sensitiveCenterTable");
					 var activitesDatail="";
					 for ( var i = 0; i < data.length; i++) {
					 if(data[i].nicknameTo==thisName){
					   if(data[i].detailContent==sensitiveWords){
					     activitesDatail+="<li><span >"+data[i].nicknameFrom+"</span>:<b> "+data[i].detailContent+"</b></li>";
					   }else{
					    activitesDatail+="<li><span>"+data[i].nicknameFrom+"</span>: "+data[i].detailContent+"</li>";
					   }
					  
					 
					 }else{
					 
					  activitesDatail+="<li><span>"+data[i].nicknameFrom+"  回复  "+data[i].nicknameTo+"</span>: "+data[i].detailContent+"</li>";
					 }
		    ...
		    var path = "<%= path%>/api/v1/communities/"+communityId+"/lifeCircle/upStatus?id="+id+"&status=dispose" ;
	     ...
	     function examine2(id,activitiesId){
			...
			$.ajax({
	      	...
	      	function pingbi(id){
	        var path = "<%= path%>/api/v1/communities/1/lifeCircle/pingbi/"+id;
				$.ajax({
			   		...
			   		<div class="sensitiveCenter" id="sensitiveCenter" >
			   			<div class="lifecircle-user">
			   				<div class="lifecircle-box clearfix">
			   					<img id="avatar_img" src="${pageContext.request.contextPath }/images/user.jpg">
			   					<div class="lifecircle-user-info">
			   					 	<p><span id="user_name_id">刘文静</span></p>
			   					 	<p id="neirong">乘风破浪，扬帆起航！让我们一起努力，世界这么大我想去看看。</p>
			   					</div>
			   				</div>
			   			</div>
			   			...
			   			<table id="sensitiveCenterTable1" align="center" width="80%">
...
<script type="text/javascript">
 ...
 getSensitiveWords(1);
 activity_left_List("life_circle_sensitive");
</script>
</html>