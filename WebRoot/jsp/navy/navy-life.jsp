...
function getActivitiesDetail(activitiesId){
	...
	$.ajax({
		url: "<%=path %>/api/v1/communities/1/lifeCircle/getLifeCireDelit?life_circle_id="+activitiesId,
		type: "GET",
		dataType:'json',
		success:function(data){
			data = data.info;
...
$(function() {
	...
	$.ajax({
		url : "<%=path %>/api/v1/communities/1/lifeCircle/getFavoriteLifeCire?pageNum=1&pageSize=10000",
		type : "GET",
		dataType : 'json',
		success : function(data) {
			...
			var html = "";
			for (var i = 0; i < data.length; i++) {
				html+="<p class=\"navy-content-item\">";
				html+="<input type=\"radio\" id=\""+data[i].lifeCircleId+"\" name=\"collection\" onclick=\"getActivitiesDetail('"+data[i].lifeCircleId+"');\">";
				html+="<label for=\""+data[i].lifeCircleId+"\" title=\""+data[i].lifeContent+"\">"+data[i].lifeContent+"</label></p>";
			}
			$("#navy_life_id").html(html);
		}
	});
	...
	<section>
		<div class="content clearfix">
			...
			<div class="right-body" style="border: none;">
				...
				<div class="clearfix">
					<div class="navy-life-tag">选择话题收藏</div>
					<div class="navy-life-content" id="navy_life_id" style="height:240px;overflow:auto;"></div>
				</div>
