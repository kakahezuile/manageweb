;(function() {
	function summary() {
		...
		$.ajax({
			url: "/api/v1/nearby/statCrazySales?communityId=" + window.nearby.communityId,
			type: "GET",
			dataType: "json",
			success : function(data) {
				...
				for (var i = 0; i < category.length; i++) {
					...
					summaryHtml += "<td><span>" + moduleInfo.userNum + "</span></td>";
					summaryHtml += "<td><span>" + moduleInfo.useNum + "</span></td>";
					summaryHtml += "</tr>";
					...
					if (i == 0) {//本日
						...
						$("#statistics-day").html("<tr class='even'>"
							...
							+ "<td><span>" + moduleClick.userCount + "</span><span class='green'>(" + moduleClick.testCount + ")</span></td>"
							+ "<td><span>" + moduleInfo.buyNum + "</span></td>"
							+ "<td><span>" + moduleInfo.userNum + "</span></td>"
							+ "<td><span>" + moduleInfo.useNum + "</span></td>"
							+ "</tr>");
	...
	function stat(type, amount) {
		...
		$.ajax({
			url: "/api/v1/nearby/statCrazySales?communityId=" + window.nearby.communityId + "&amount=" + amount + "&type=" + typeMap[type],
			type: "GET",
			dataType: "json",
			success : function(data) {
				var infoes = data.info,
				...
				for (var i = 0; i < click.length; i++) {
					...
					html += "<td><span>" + (moduleInfo.userNum || "0") + "</span></td>";
					html += "<td><span>" + (moduleInfo.useNum || "0") + "</span></td>";
					html += "</tr>";
				}
	...
	function statSelect() {
		...
		$.ajax({
			url: "/api/v1/nearby/statCrazySales?communityId=" + window.nearby.communityId + "&start=" + start + "&end=" + end,
			type: "GET",
			dataType: "json",
			success : function(data) {
				var infoes = data.info,
