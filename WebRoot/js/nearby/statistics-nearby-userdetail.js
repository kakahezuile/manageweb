;(function() {
		...
		$.ajax({
			...
			success : function(data) {
				var infoes = data.info,
					html = "<tr><th>用户</th><th>联系方式</th><th>抢购活动名</th><th>发布活动商家</th><th>抢购量</th><th>验码量</th></tr>",
					info,
					i;
				if (infoes.length == 0) {
					...
					for (i = 0; i < infoes.length; i++) {
						info = infoes[i];
						html += "<tr class='" + (i / 2 == 0 ? "even" : "odd") + "'><td>" + info.date + "</td><td>" + info.download + "</td><td>" + info.register + "</td><td>" + info.pub + "</td><td>" + info.money + "</td><td>新加列</td</tr>";
					}
				}
	...
	function summary() {
		$.ajax({
			...
			success : function(data) {
				var infoes = data.info,
					html = "<tr><th>日期</th><th>点击量</th><th>点击人数</th><th>购买量</th><th>购买人数</th><th>验码量</th></tr>",
					info,
					i;
				if (infoes.length == 0) {
					html += "<tr class='even'><td colspan='5' style='text-align:center;color:red;'>没有查询到数据!</td></tr>";
