...
function getTimeQuantumTop(sort, startTime, endTime,type) {
	schedule();
	var path =  "/api/v1/communities/"+communityId+"/users/1/orderHistories/getTimeQuantumTop?sort="
	...
	$
			.ajax({
				...
				success : function(data) {
					...
					var avg=0;
					if(zhong!=0){
						ratio = ((max / zhong) * 100).toFixed(2);
						avg=(((data.serveTime30*30)+(data.serveTime45*45)+(data.serveTime15*15))/zhong).toFixed(2);
					}
					...
					document.getElementById("turnover_avg").innerHTML=(data.avgServeTime/60).toFixed(2)+"min";
...
function getTimeQuantumList(sort, pageNum, startTime, endTime) {
	schedule();
	var path =  "/api/v1/communities/"+communityId+"/users/1/orderHistories/getTimeQuantumList?pageSize=50&pageNum="
