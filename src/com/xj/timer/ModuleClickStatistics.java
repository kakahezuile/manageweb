...
public class ModuleClickStatistics {
	...
	public void execute() {
		try {
			...
			List<StatisticsTime> statisticsTimeList = statisticsTimeService.selectStatisticsTimeType("rest");
			...
			for (int i = 1; i < dateList.size() - 2; i++) {
				int time = (int)(dateList.get(i).getTime() / 1000);
				for (Communities communities : l) {
					statService.getClickMobule(time, (int) (((dateList.get(i).getTime() + (24 * 60 * 60 * 1000)) / 1000) - 1) , communities.getCommunityId());
				}
				statisticsTime.setStatisticsUserTime(time);
